...
impl Bridge {
    ...
    /// Tick attempts to hand a packet from the queue for each directions, to
    /// readers, if there are waiting on the queue. If there's no reader, it
    ...
    /// Process repeats tick() calls until no more outstanding packet in the
    /// queues.
