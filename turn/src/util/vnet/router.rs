...
pub struct RouterConfig {
    ...
    pub cidr: String,
    // static_ips is an array of static IP addresses to be assigned for this
    // router. If no static IP address is given, the router will
...
pub struct RouterInternal {
    ...
    pub(crate) last_id: u8,                        /* requires mutex [x], used to assign the
                                                    * last
                                                    * digit of IPv4 address */
}
...
impl Nic for Router {
    ...
    async fn set_router(&self, parent: Arc<Mutex<Router>>) -> Result<()> {
        ...
        // when this method is called, one or more IP address has already been
        // assigned by the parent router.
...
impl Router {
    ...
    // after parent.add_router(child), also call child.set_router(parent) to set
    // child's parent router
    pub async fn add_router(&mut self, child: Arc<Mutex<Router>>) -> Result<()> {
        // Router is a NIC. Add it as a NIC so that packets are routed to this
        // child router.
    ...
    // AddNet ...
    // after router.add_net(nic), also call nic.set_router(router) to set nic's
    // router
    ...
    // AddHost adds a mapping of hostname and an IP address to the local
    // resolver.
    ...
    // You may add more than one filter. The filters are called in the order of
    // this method call. If a chunk is dropped by a filter, subsequent
    // filter will not receive the chunk.
        ...
        loop {
            ...
            if let Some(c) = queue.peek().await {
                ...
                if c.get_timestamp().duration_since(cut_off).is_ok() {
                    // There is one or more chunk in the queue but none of them
                    // are due. Calculate the next sleep
                    // duration here.
