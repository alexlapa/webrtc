...
pub enum EndpointDependencyType {
    // EndpointIndependent means the behavior is independent of the endpoint's
    // address or port
    #[default]
    EndpointIndependent,
    // EndpointAddrDependent means the behavior is dependent on the endpoint's
    // address
    EndpointAddrDependent,
    // EndpointAddrPortDependent means the behavior is dependent on the
    // endpoint's address and port
...
pub enum NatMode {
    ...
    #[default]
    Normal,
    // NATModeNAT1To1 exhibits 1:1 DNAT where the external IP address is
    // statically mapped to a specific local IP address with port number
    // is preserved always between them. When this mode is selected,
    // mapping_behavior, filtering_behavior, port_preservation and
...
pub(crate) struct NatConfig {
    ...
    pub(crate) local_ips: Vec<IpAddr>,  /* local IPv4, required only when
                                         * the mode is NATModeNAT1To1 */
}
...
pub(crate) struct Mapping {
    ...
    bound: String,  /* key: "[<remote-ip>[:
                     * <remote-port>]]" */
    filters: Arc<Mutex<HashSet<String>>>, /* key: "[<remote-ip>[:
                                           * <remote-port>]]" */
...
impl Default for Mapping {
    fn default() -> Self {
        Mapping {
            ...
            bound: String::new(),  /* key: "[<remote-ip>[:
                                    * <remote-port>]]" */
            filters: Arc::new(Mutex::new(HashSet::new())), /* key: "[<remote-ip>[:
                                                            * <remote-port>]]" */
...
pub(crate) struct NetworkAddressTranslator {
    ...
    pub(crate) local_ips: Vec<IpAddr>,  /* local IPv4, required only when
                                         * the mode is NATModeNAT1To1 */
    // key: "<proto>:<local-ip>:<local-port>[:remote-ip[:remote-port]]
    pub(crate) outbound_map: Arc<Mutex<HashMap<String, Arc<Mapping>>>>,
    // key: "<proto>:<mapped-ip>:<mapped-port>"
    pub(crate) inbound_map: Arc<Mutex<HashMap<String, Arc<Mapping>>>>,
        ...
        if from.network() == UDP_STR {
            if self.nat_type.mode == NatMode::Nat1To1 {
                ...
                if let Some(m) = self.find_inbound_mapping(&i_key).await {
                    ...
                    // outgoing   UDP traffic.  However,
                    // allowing inbound refresh may allow an
                    //   external attacker or misbehaving application to keep a
                    // Mapping   alive indefinitely.  This
                    // may be a security risk.  Also, if the
                    //   process is repeated with different ports, over time, it
                    // could   use up all the ports on the
                    // NAT.
